...
# Pure regulatory compliance paperwork (UK Takeover Code disclosure forms) - not
# news at all, just legally-required filings. Finnhub's 'merger' category republishes
# these constantly and they have zero trading relevance. Hard-excluded regardless
# of any other keyword match.
NOISE_EXCLUSION_PHRASES = [
    "form 8.3", "form 8.5", "rule 8.3 of the takeover code", "rule 8.5 of the takeover code",
    "public dealing disclosure", "public opening position disclosure",
    "exempt principal trader", "irish takeover panel",
]


_ENGLISH_COMMON_WORDS = {
    "the", "and", "for", "with", "to", "of", "in", "on", "is", "are", "was", "will",
    "that", "this", "from", "has", "have", "its", "by", "as", "at", "an", "be",
}


def _looks_non_english(text):
    """No heavy language-detection dependency needed - English news headlines/
    summaries reliably contain several extremely common short English words.
    Foreign-language press releases (GlobalNewswire/BusinessWire publish in
    dozens of languages) essentially never do. Cheap and reliable enough for
    this purpose without adding a new package."""
    words = set(text.lower().split())
    hits = len(words & _ENGLISH_COMMON_WORDS)
    return len(text) > 80 and hits < 3
...
def classify_article(headline, summary, body_text):
    ...
    if _looks_non_english(f"{headline} {summary}"):
        return {"is_relevant": False}

    if any(phrase in haystack for phrase in NOISE_EXCLUSION_PHRASES):
        return {"is_relevant": False}
