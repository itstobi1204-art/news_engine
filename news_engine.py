...
RSS_FEEDS = {
    ...
    "https://www.cnbc.com/id/15837362/device/rss/rss.html": "cnbc",
...
def scrape_article_details(article_url, browser=None):
    ...
    # Playwright fallback for JS rendering
    if len(body_text) < 200 and browser is not None:
        page = None
        try:
            page = browser.new_page(user_agent=HEADERS["User-Agent"])
            page.set_default_timeout(12000)
            page.goto(article_url, wait_until="domcontentloaded", timeout=12000)
...
def fetch_rss_articles(feed_url, source_name, browser=None):
    """Fetches RSS XML with automatic Headless Browser fallback for Cloudflare bypass."""
    raw_xml = None

    # Step 1: Rapid HTTP fetch
    try:
    ...
    # Step 2: Headless Chromium fallback (Bypasses Cloudflare / 403 blocks)
    if not raw_xml and browser is not None:
        page = None
        try:
            page = browser.new_page(user_agent=HEADERS["User-Agent"])
            page.set_default_timeout(10000)
            page.goto(feed_url, wait_until="domcontentloaded", timeout=10000)
            raw_xml = page.content()
        except Exception as e:
            print(f"Browser RSS fetch failed ({source_name}): {e}")
        finally:
            if page:
                page.close()

    if not raw_xml:
        print(f"RSS fetch warning ({source_name}): Host unreachable or blocked.")
        return []
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            if hasattr(entry, "published_parsed") and entry.published_parsed:
                ...
                pub_ts = int(calendar.timegm(entry.updated_parsed))
...
def process_live_news(state, now_ts, browser=None):
    try:
        ...
        for feed_url, source_name in RSS_FEEDS.items():
            for item in fetch_rss_articles(feed_url, source_name, browser):
                art_id = str(item.get("id") or item.get("url"))
                if art_id not in seen_in_batch:
                    seen_in_batch.add(art_id)
                    articles.append(item)
...
def main():
    print("==================================================")
    print("      Starting Resilience-Upgraded Engine...     ")
    print("==================================================")

    state = load_state()

    # Launch Playwright Browser Early (used for Cloudflare bypass & JS parsing)
    playwright_ctx = None
    browser = None
    try:
        ...
        print("Playwright Browser loaded for Cloudflare bypass.")
    except Exception as e:
        print(f"Playwright initialization warning: {e}")

    # Seed all current news on boot
    try:
        seeded = 0
        for category in FINNHUB_CATEGORIES:
            for item in fetch_finnhub_articles(category):
    ...
    print("\n[ACTIVE] Engine listening for live market news across all RSS feeds...\n")
